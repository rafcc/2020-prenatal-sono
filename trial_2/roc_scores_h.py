...
normal_image_path = "/share/fujitsu/benchmark/dataset/h_validation/normal"
chd_image_path = "/share/fujitsu/benchmark/dataset/h_validation/abnormal
...
if __name__ == "__main__":
    ...
    score = []
    dobjs = []
    roc_scores_file = folder_name + "h_roc_scores.csv"
    roc_lines = []
    for nc in range(2):
        if(nc == 0):
            image_path = normal_image_path + "/n"
            out_file_name = folder_name + "h_tests/n"
        if(nc == 1):
            image_path = chd_image_path + "/chd"
            out_file_name = folder_name + "h_tests/chd"
        for j in range(1,2000):
            filename = out_file_name + str(j) + ".csv"
            lines = []
            score = []
            dobjs = []
            count_h = [0,0,0,0,0,0,0,0]
            count_h_sum = 0
            for i in range(1000):
                scores = []
                dobjs = []
                ...
                if(objs[0] >= 0.01):
                    count_h[0] += 1
                if(objs[1] >= 0.01 or objs[18] >= 0.01):
                    count_h[1] += 1
                if(objs[2] >= 0.01):
                    count_h[2] += 1
                if(objs[3] >= 0.01):
                    count_h[3] += 1
                if(objs[4] >= 0.01 or objs[19] >= 0.01):
                    count_h[4] += 1
                if(objs[5] >= 0.01):
                    count_h[5] += 1
                if(objs[6] >= 0.01):
                    count_h[6] += 1
                if(objs[7] >= 0.01 or objs[20] >= 0.01):
                    count_h[7] += 1
            if lines == []:
                continue
            for ic in range(8):
                count_h_sum += count_h[ic]
            roc_lines.append([nc,1.0 - count_h_sum/160.0,j])
            lib.write_csv(lines,filename)
        lib.write_csv(roc_lines,roc_scores_file)
